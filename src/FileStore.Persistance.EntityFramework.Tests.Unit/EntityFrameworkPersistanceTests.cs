...
namespace FileStore.Persistance.EntityFramework.Tests.Unit
{
    public class EntityFrameworkPersistanceTests
    {
        ...
        [Fact]
        public void Persist_SameIdAndVersionAllreadyExists_DeletesFile()
        {
            var id = Guid.NewGuid();
            var version = 5;
            var existingFile = new StoredFile { FileId = id, Version = version, Data = new byte[] { 1, 2, 3 } };
            var currentFiles = new List<StoredFile> { existingFile };
            A.CallTo(() => context.StoredFiles).Returns(Aef.FakeDbSet(currentFiles));

            persistance.Persist(id, new StoreFile(), version);

            Assert.DoesNotContain(currentFiles, p => p.Data == existingFile.Data);
            Assert.Equal(1, currentFiles.Count);
        }
